...
@section('meta_description', 'Bagan struktur organisasi, pimpinan pengelola, dewan guru, dan unit kerja SMA Islam Terpadu Ishlahul Ummah Prabumulih.')
...
<div class="bg-gradient-to-r from-emerald-950 via-[#00913e] to-emerald-900 text-white py-12">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <p class="text-sm text-emerald-100 mt-2 font-light max-w-2xl">
            Susunan pimpinan pengelola, wakil kepala sekolah, unit koordinasi tahfidz, dan tata usaha SMA IT Ishlahul Ummah Prabumulih.
        </p>
    </div>
</div>
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14 space-y-16">
                ...
                <div class="bg-gray-50/80 rounded-2xl p-6 border border-gray-200/80 hover:border-emerald-300 hover:shadow-lg transition transform hover:-translate-y-1 text-center flex flex-col justify-between space-y-4 group">
                    <div class="space-y-3">
                        ...
                        <div>
                            ...
                            <h3 class="font-extrabold text-sm sm:text-base text-gray-900 group-hover:text-[#00913e] transition leading-snug">
                                {{ $leader['name'] }}
                            </h3>
    ...
    <section class="bg-white p-8 sm:p-12 rounded-3xl shadow-xl border border-gray-100 reveal-fade-up">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
            ...
            <a href="{{ route('bidang.index') }}" class="inline-flex items-center text-xs font-bold text-[#00913e] hover:text-orange-500 flex-shrink-0">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($bidangs as $b)
                <a href="{{ route('bidang.show', $b->slug) }}" class="p-5 rounded-2xl border border-gray-100 hover:border-[#00913e] hover:shadow-md transition group bg-gray-50/80 flex items-start space-x-4">
                    <div class="w-12 h-12 rounded-xl bg-emerald-100 text-[#00913e] group-hover:bg-[#00913e] group-hover:text-white flex items-center justify-center text-lg flex-shrink-0 transition shadow-inner">
                        <i class="fa-solid fa-school"></i>
                    </div>
                    <div>
                        <h3 class="font-bold text-sm text-gray-900 group-hover:text-[#00913e] transition">{{ $b->name }}</h3>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            @forelse($dpcs as $dpc)
                <div class="p-4 rounded-xl border border-gray-100 bg-gray-50/80 flex items-center space-x-3 hover:border-emerald-300 transition">
                    <div class="w-9 h-9 rounded-lg bg-[#00913e] text-white flex items-center justify-center font-bold text-xs flex-shrink-0 shadow">
                        <i class="fa-solid fa-star"></i>
                    </div>
                    <div class="overflow-hidden">
                        <h3 class="font-bold text-xs sm:text-sm text-gray-900 truncate">{{ $dpc->name }}</h3>
                        <span class="text-[11px] text-gray-500 block truncate">{{ $dpc->address ?: 'Program Unggulan Ishum' }}</span>
                    </div>
                </div>
            @empty
                <div class="col-span-full text-center py-8 text-gray-400">
