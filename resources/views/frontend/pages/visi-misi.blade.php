...
@section('title', ($page?->meta_title ?: ($page?->title ?: 'Visi dan Misi')) . ' - ' . ($siteSettings['site_name'] ?? 'Pondok Pesantren Raudhatul Ulum Sakatiga'))
@section('meta_description', $page?->meta_description ?: 'Visi dan Misi resmi Pondok Pesantren Raudhatul Ulum Sakatiga: Menjadi basis kaderisasi generasi terbaik (Khoiru Ummah) yang bermanfaat luas dan berdaya saing global.')
...
<div class="bg-gradient-to-r from-emerald-950 via-[#00913e] to-emerald-900 text-white py-12">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <h1 class="text-3xl sm:text-4xl font-extrabold tracking-tight">{{ $page?->title ?: 'Visi, Misi &amp; Jati Diri Santri' }}</h1>
        <p class="text-sm text-emerald-100 mt-2 font-light max-w-2xl">
            {{ $page?->excerpt ?: 'Komitmen luhur Pondok Pesantren Raudhatul Ulum Sakatiga dalam membina generasi Khairu Ummah berakhlak Qur\'ani dan berwawasan global.' }}
        </p>
    </div>
</div>
            ...
            @if(!empty($page?->content) && strlen(trim(strip_tags($page->content))) > 0)
                <div class="bg-white rounded-3xl p-8 sm:p-10 shadow-xl border border-gray-100 reveal-fade-up">
                    <div class="flex items-center space-x-3 mb-6">
                        ...
                        <div>
                            <span class="text-xs font-bold text-school-green uppercase tracking-wider block">Pedoman Resmi</span>
                            <h2 class="text-2xl sm:text-3xl font-extrabold text-gray-900 tracking-tight">{{ $page->title }}</h2>
                        </div>
                    </div>
                    <div class="w-16 h-1 bg-[#00913e] rounded-full mb-8"></div>

                    <div class="prose-content text-gray-800 text-sm sm:text-base leading-relaxed">
                        {!! $page->content !!}
                    </div>
                </div>
            @else
