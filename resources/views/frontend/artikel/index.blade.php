...
@section('title', 'Berita & Artikel - SMA IT Plus Robbani')
@section('meta_description', 'Kumpulan berita sekolah, prestasi siswa, kegiatan akademik, tahfidz, dan artikel edukasi SMA IT Plus Robbani Indralaya Ogan Ilir.')
...
<div class="bg-gradient-to-r from-emerald-950 via-[#0d6b38] to-emerald-900 text-white py-12">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <p class="text-sm text-emerald-100 mt-2 font-light max-w-2xl">
            Informasi kegiatan santri, prestasi akademik & tahfidz, serta kabar terkini SMA IT Plus Robbani.
        </p>
    </div>
</div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            <div class="flex items-center space-x-2 overflow-x-auto pb-2 scrollbar-none text-xs">
                <a href="{{ route('artikel.index') }}" class="px-3.5 py-1.5 rounded-full font-medium whitespace-nowrap transition {{ !request('kategori') && !request('tag') ? 'bg-[#0d6b38] text-white shadow-sm' : 'bg-gray-100 text-gray-600 hover:bg-emerald-50 hover:text-[#0d6b38]' }}">
                    Semua
                </a>
                @foreach($categories->take(8) as $cat)
                    <a href="{{ route('artikel.index', ['kategori' => $cat->slug]) }}" class="px-3.5 py-1.5 rounded-full font-medium whitespace-nowrap transition {{ request('kategori') == $cat->slug ? 'bg-[#0d6b38] text-white shadow-sm' : 'bg-gray-100 text-gray-600 hover:bg-emerald-50 hover:text-[#0d6b38]' }}">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <article class="bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-lg transition-all duration-300 flex flex-col group reveal-fade-up delay-{{ $idx % 4 }}">
                        <a href="{{ route('artikel.show', $post->slug) }}" class="block relative h-48 overflow-hidden bg-gray-100">
                            @if($post->featured_image)
                                <img src="{{ $post->featured_image }}" alt="{{ $post->title }}" class="w-full h-full object-cover group-hover:scale-105 transition duration-500" onerror="this.src='/uploads/campus-robbani.jpg'">
                            @else
                                <img src="/uploads/campus-robbani.jpg" alt="{{ $post->title }}" class="w-full h-full object-cover">
                            @endif
                            @if($post->categories->isNotEmpty())
                                <span class="absolute top-3 left-3 bg-[#0d6b38] text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow-sm">
                        ...
                        <div class="p-5 flex-grow flex flex-col justify-between space-y-3">
                            <div>
                                <div class="flex items-center text-[11px] text-gray-400 space-x-2 mb-2">
                                    <span><i class="fa-regular fa-calendar mr-1 text-[#0d6b38]"></i>{{ $post->published_at ? $post->published_at->translatedFormat('d M Y') : '-' }}</span>
                                    <span>&bull;</span>
                                    <span><i class="fa-regular fa-eye mr-1"></i>{{ number_format($post->views_count) }}</span>
                                </div>
                                <h2 class="font-bold text-gray-900 text-sm sm:text-base line-clamp-2 group-hover:text-[#0d6b38] transition">
                            ...
                            <div class="pt-2 border-t border-gray-50 flex items-center justify-between text-xs font-semibold text-[#0d6b38] group-hover:text-orange-600">
                ...
                @empty
                    <div class="col-span-2 text-center py-16 bg-white rounded-2xl border border-gray-100">
                        ...
                        <a href="{{ route('artikel.index') }}" class="inline-block mt-4 text-xs font-semibold text-[#0d6b38] hover:underline">
        ...
        <div class="space-y-8">
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                ...
                <form action="{{ route('artikel.index') }}" method="GET" class="relative">
                    <input type="text" name="q" placeholder="Ketik kata kunci..." value="{{ request('q') }}" class="w-full bg-gray-50 text-xs text-gray-800 rounded-xl pl-4 pr-10 py-3 border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#0d6b38]">
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                ...
                <ul class="space-y-2 text-xs">
                    @foreach($categories->take(12) as $cat)
                        <li>
                            <a href="{{ route('artikel.index', ['kategori' => $cat->slug]) }}" class="flex items-center justify-between py-1.5 px-2 rounded-lg hover:bg-emerald-50 hover:text-[#0d6b38] transition {{ request('kategori') == $cat->slug ? 'text-[#0d6b38] font-bold bg-emerald-50' : 'text-gray-600' }}">
