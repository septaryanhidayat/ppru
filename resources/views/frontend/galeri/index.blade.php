...
@section('title', 'Galeri Dokumentasi Foto - Pondok Pesantren Raudhatul Ulum Sakatiga')
@section('meta_description', 'Dokumentasi foto kegiatan belajar mengajar, tahfidz, laboratorium sains, wisuda, dan prestasi santri Pondok Pesantren Raudhatul Ulum Sakatiga.')
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
        @forelse($galleryImages as $idx => $img)
            <div class="group relative rounded-2xl overflow-hidden bg-gray-100 shadow-md hover:shadow-2xl transition transform hover:-translate-y-1 aspect-square cursor-pointer reveal-fade-up delay-{{ $idx % 4 }}"
                ...
                <img src="{{ $img->featured_image }}" alt="{{ $img->title }}" class="w-full h-full object-cover group-hover:scale-110 transition duration-500" onerror="this.onerror=null;this.src='/images/hero-1.webp'">
