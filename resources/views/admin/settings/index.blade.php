...
<div class="max-w-5xl space-y-6">
    ...
    <form action="{{ route('admin.settings.update') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        {{-- 7. PENGATURAN POPUP BANNER PROMOSI (HALAMAN BERANDA) --}}
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-6 py-4 bg-gray-50/80 border-b border-gray-100 flex items-center justify-between">
                <div class="flex items-center space-x-2.5">
                    <span class="w-7 h-7 rounded-lg bg-red-100 text-[#da251c] flex items-center justify-center text-xs font-bold">7</span>
                    <div>
                        <h2 class="font-bold text-sm text-gray-900">Popup Banner Promosi (Halaman Beranda)</h2>
                        <p class="text-[11px] text-gray-500">Banner modal yang muncul otomatis ketika pengunjung membuka halaman depan website</p>
                    </div>
                </div>
                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold {{ ($settings['popup_active'] ?? '1') === '1' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-gray-100 text-gray-500 border border-gray-200' }}">
                    <i class="fa-solid fa-bullhorn mr-1.5"></i> {{ ($settings['popup_active'] ?? '1') === '1' ? 'Popup Aktif' : 'Popup Nonaktif' }}
                </span>
            </div>
            ...
            <div class="p-6 space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="rounded-2xl border-2 border-dashed border-gray-200 p-2 bg-gray-50 flex flex-col items-center justify-center overflow-hidden">
                            @if(!empty($settings['popup_image']))
                                <img src="{{ asset($settings['popup_image']) }}" alt="Popup Flyer" class="max-h-72 w-auto object-contain rounded-xl shadow-sm">
                            @else
                                <div class="py-12 text-center text-gray-400">
                                    <i class="fa-regular fa-image text-3xl mb-2"></i>
                                    <p class="text-xs">Belum ada gambar banner</p>
                                </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="md:col-span-2 space-y-4">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                                    Status Popup Beranda <span class="text-red-500">*</span>
                                </label>
                                <select name="popup_active" class="w-full bg-gray-50 text-xs text-gray-800 rounded-xl px-4 py-3 border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#00913e] font-semibold">
                                    <option value="1" {{ ($settings['popup_active'] ?? '1') === '1' ? 'selected' : '' }}>🟢 Aktif (Tampilkan di Beranda)</option>
                                    <option value="0" {{ ($settings['popup_active'] ?? '1') === '0' ? 'selected' : '' }}>🔴 Nonaktif (Matikan Popup)</option>
                                </select>
                                <p class="text-[10px] text-slate-400 mt-1">Jika dinonaktifkan, popup tidak akan tampil kepada pengunjung sama sekali.</p>
                            </div>

                            <div>
                                <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                                    Upload / Ganti Gambar Banner (Pilih File)
                                </label>
                                <input type="file" name="popup_image_file" accept="image/*" class="w-full text-xs text-gray-500 file:mr-3 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-red-50 file:text-[#da251c] hover:file:bg-red-100 bg-gray-50 rounded-xl border border-gray-200">
                                <p class="text-[10px] text-slate-400 mt-1">Otomatis dikonversi ke WebP untuk loading super cepat.</p>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="popup_title" value="{{ $settings['popup_title'] ?? 'Penerimaan Peserta Didik Baru (PPDB) TP 2025/2026' }}" class="w-full bg-gray-50 text-xs text-gray-800 rounded-xl px-4 py-3 border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#00913e] font-medium" placeholder="Contoh: Penerimaan Peserta Didik Baru (PPDB)">
                        </div>

                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                                    URL Tujuan Klik / Tombol
                                </label>
                                <input type="text" name="popup_link" value="{{ $settings['popup_link'] ?? '/ppdb' }}" class="w-full bg-gray-50 text-xs text-gray-800 rounded-xl px-4 py-3 border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#00913e] font-medium" placeholder="Contoh: /ppdb atau https://...">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="popup_button_text" value="{{ $settings['popup_button_text'] ?? 'Daftar PPDB Sekarang' }}" class="w-full bg-gray-50 text-xs text-gray-800 rounded-xl px-4 py-3 border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#00913e] font-medium" placeholder="Contoh: Daftar PPDB Sekarang">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="popup_target" class="w-full bg-gray-50 text-xs text-gray-800 rounded-xl px-4 py-3 border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#00913e] font-medium">
                                <option value="_self" {{ ($settings['popup_target'] ?? '_self') === '_self' ? 'selected' : '' }}>Halaman yang Sama (_self)</option>
                                <option value="_blank" {{ ($settings['popup_target'] ?? '_self') === '_blank' ? 'selected' : '' }}>Buka di Tab Baru (_blank)</option>
                            </select>
                        </div>
                    </div>
                </div>
