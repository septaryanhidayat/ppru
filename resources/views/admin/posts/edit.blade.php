...
@php
    ...
    // URL preview publik sesuai tipe postingan
    $publicUrl = match($post->type) {
        'prestasi' => route('prestasi.show', $post->slug),
        'page' => route('page.show', $post->slug),
        default => route('artikel.show', $post->slug),
    };
@endphp
...
<div class="max-w-5xl bg-white rounded-3xl shadow-xs border border-slate-200/80 p-6 sm:p-8">
    {{-- Header Actions --}}
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-6 border-b border-slate-100 mb-6">
        <div>
            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Memperbarui Konten</span>
            <h2 class="text-base font-black text-slate-800 line-clamp-1">{{ $post->title }}</h2>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ $publicUrl }}" target="_blank" class="inline-flex items-center gap-1.5 bg-slate-100 hover:bg-emerald-50 hover:text-[#00843d] text-slate-700 font-bold text-xs px-4 py-2.5 rounded-xl border border-slate-200 transition" title="Lihat bagaimana artikel ini tampil di website publik">
                <i class="fa-solid fa-arrow-up-right-from-square text-xs text-[#00843d]"></i>
                <span>Lihat di Web Publik</span>
            </a>
            <a href="{{ route('admin.posts.index', ['type' => $post->type]) }}" class="inline-flex items-center gap-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs px-3.5 py-2.5 rounded-xl transition">
    ...
    <form action="{{ route('admin.posts.update', $post->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="title" id="title" required value="{{ old('title', $post->title) }}" class="w-full bg-slate-50 text-xs sm:text-sm text-slate-800 rounded-xl px-4 py-3 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#00843d] transition font-medium">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-5 bg-slate-50/70 p-5 rounded-2xl border border-slate-200/70">
            {{-- Tanggal Terbit Fleksibel (Bisa Diedit Kapanpun) --}}
            <div>
                ...
                <input type="datetime-local" name="published_at" id="published_at" value="{{ old('published_at', $post->published_at ? $post->published_at->format('Y-m-d\TH:i') : now()->format('Y-m-d\TH:i')) }}" class="w-full bg-white text-xs text-slate-800 rounded-xl px-3 py-2.5 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#00843d]">
                <p class="text-[10px] text-slate-400 mt-1">Tanggal ini menentukan urutan rilis berita di website publik (WIB).</p>
            ...
            <div>
                ...
                <select name="status" id="status" class="w-full bg-white text-xs text-slate-800 rounded-xl px-3 py-2.5 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#00843d]">
                    <option value="publish" {{ old('status', $post->status) === 'publish' ? 'selected' : '' }}>Publikasikan Langsung</option>
                    <option value="draft" {{ old('status', $post->status) === 'draft' ? 'selected' : '' }}>Simpan Sebagai Draft</option>
                </select>
                <div class="mt-2 flex items-center gap-2">
                    <input type="checkbox" name="is_featured" id="is_featured" value="1" {{ old('is_featured', $post->is_featured) ? 'checked' : '' }} class="rounded border-slate-300 text-[#00843d] focus:ring-[#00843d]">
            ...
            <div>
                ...
                <select name="author_id" id="author_id" class="w-full bg-white text-xs text-slate-800 rounded-xl px-3 py-2 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#00843d] mb-2">
                    @foreach($users as $usr)
                        <option value="{{ $usr->id }}" {{ old('author_id', $post->author_id) == $usr->id ? 'selected' : '' }}>
                            {{ $usr->name }} ({{ $usr->role ?? 'Staf' }})
                        </option>
                    @endforeach
                </select>
                <input type="text" name="author_name" id="author_name" value="{{ old('author_name', $post->author_name) }}" placeholder="Atau ketik nama kustom (Opsional)..." class="w-full bg-white text-xs text-slate-800 rounded-xl px-3 py-1.5 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#00843d]">
            </div>
        </div>
        ...
        <div class="bg-slate-50/70 p-5 rounded-2xl border border-slate-200/70 space-y-4">
            <div>
                <label for="featured_image" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">
                    <i class="fa-regular fa-image text-[#00843d] mr-1"></i> Ganti Gambar Utama (Featured Image)
                </label>

                @if($post->featured_image)
                    <div class="mb-3 flex items-center space-x-4 p-3 bg-white rounded-xl border border-slate-200 shadow-2xs">
                        <img src="{{ $post->featured_image }}" alt="" class="w-24 h-16 object-cover rounded-lg">
                        <div class="text-xs">
                            <span class="text-slate-400 block font-semibold text-[10px] uppercase tracking-wider">Gambar saat ini:</span>
                            <span class="font-mono text-[11px] text-slate-800 block truncate max-w-md">{{ $post->featured_image }}</span>
                        </div>
                    </div>
                @endif
                ...
                <div class="bg-white border-2 border-dashed border-emerald-200/80 rounded-2xl p-4 text-center hover:bg-emerald-50/30 transition">
                    ...
                    <p class="text-[11px] text-[#00843d] font-medium mt-2 flex items-center justify-center">
                        ...
                        Bila memilih gambar baru, otomatis dikonversi ke format <strong>WebP</strong> berukuran ringan dengan kualitas tajam.
                    </p>
                </div>
            ...
            <div>
                ...
                <input type="text" name="featured_image_caption" id="featured_image_caption" value="{{ old('featured_image_caption', $post->featured_image_caption) }}" placeholder="Contoh: Foto: Dokumentasi Tim Humas PPRU Sakatiga" class="w-full bg-white text-xs text-slate-800 rounded-xl px-4 py-2.5 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#00843d]">
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        @foreach($categories as $cat)
                            ...
                            <label class="flex items-center justify-between text-slate-700 hover:text-[#00843d] cursor-pointer py-0.5 {{ $isKhutbah ? 'bg-emerald-50/60 p-1.5 rounded-lg border border-emerald-200/80' : '' }}">
                                <div class="flex items-center space-x-2">
                                    <input type="checkbox" name="categories[]" value="{{ $cat->id }}" {{ in_array($cat->id, old('categories', $selectedCategories)) ? 'checked' : '' }} class="rounded border-slate-300 text-[#00843d] focus:ring-[#00843d]">
            ...
            <div class="space-y-4 bg-slate-50/70 p-5 rounded-2xl border border-slate-200/70">
                <div>
                    ...
                    <input type="text" name="tags" id="tags" value="{{ old('tags', $selectedTags) }}" placeholder="Contoh: santri, tahfidz, prestasi, ogan ilir" class="w-full bg-white text-xs text-slate-800 rounded-xl px-4 py-2.5 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#00843d]">
                ...
                <div>
                    <label for="excerpt" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">
                        <i class="fa-solid fa-align-left text-[#00843d] mr-1"></i> Ringkasan (Excerpt)
                    </label>
                    <textarea name="excerpt" id="excerpt" rows="3" placeholder="Ringkasan singkat untuk tampilan kartu berita." class="w-full bg-white text-xs text-slate-800 rounded-xl p-3 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#00843d]">{{ old('excerpt', $post->excerpt) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div>
            <div class="flex items-center justify-between mb-2">
                ...
                <div class="flex items-center space-x-3 text-[11px] text-slate-500 font-medium">
                    ...
                    <span>&bull;</span>
                    <span><i class="fa-regular fa-clock text-[#00843d] mr-1"></i><span id="reading_time_display">{{ $post->reading_time }}</span></span>
                </div>
            </div>
        ...
        <div class="border border-slate-200 rounded-2xl overflow-hidden bg-slate-50/50">
            <button type="button" onclick="toggleSeoSection()" class="w-full p-4 flex items-center justify-between text-left font-bold text-xs text-slate-800 uppercase tracking-wider hover:bg-slate-100 transition">
                <span class="flex items-center gap-2">
                    ...
                    <span>Optimasi Mesin Pencari (SEO Google)</span>
                </span>
            ...
            <div id="seo_section_body" class="p-5 border-t border-slate-200 space-y-4 bg-white hidden">
                ...
                <div class="p-4 bg-slate-50 rounded-2xl border border-slate-200">
                    ...
                    <div class="text-xs text-slate-500 font-mono line-clamp-1" id="seo_preview_url">{{ $publicUrl }}</div>
                    <div class="text-sm font-bold text-[#1a0dab] hover:underline cursor-pointer line-clamp-1 mt-0.5" id="seo_preview_title">{{ $post->meta_title ?: $post->title }}</div>
                    <div class="text-xs text-[#4d5156] line-clamp-2 mt-1" id="seo_preview_desc">{{ $post->meta_description ?: ($post->excerpt ?: Str::limit(strip_tags($post->content), 150)) }}</div>
                </div>

                <div>
                    <div class="flex items-center justify-between mb-1">
                        <label for="meta_title" class="block text-xs font-bold text-slate-700">Meta Title</label>
                        <span id="meta_title_counter" class="text-[10px] text-slate-400">{{ strlen($post->meta_title ?? '') }} / 70 karakter</span>
                    </div>
                    <input type="text" name="meta_title" id="meta_title" maxlength="70" value="{{ old('meta_title', $post->meta_title) }}" placeholder="Judul khusus SEO Google (bila ingin beda dari judul berita)..." class="w-full bg-slate-50 text-xs text-slate-800 rounded-xl px-4 py-2.5 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#00843d]">
                </div>

                <div>
                    <div class="flex items-center justify-between mb-1">
                        <label for="meta_description" class="block text-xs font-bold text-slate-700">Meta Description</label>
                        <span id="meta_desc_counter" class="text-[10px] text-slate-400">{{ strlen($post->meta_description ?? '') }} / 160 karakter</span>
                    </div>
                ...
                <div>
                    ...
                    <input type="text" name="meta_keywords" id="meta_keywords" value="{{ old('meta_keywords', $post->meta_keywords) }}" placeholder="pesantren, ppru sakatiga, berita sekolah, pendidikan islam" class="w-full bg-slate-50 text-xs text-slate-800 rounded-xl px-4 py-2.5 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#00843d]">
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center space-x-3 pt-4 border-t border-slate-100">
            ...
            <a href="{{ route('admin.posts.index', ['type' => $post->type]) }}" class="bg-slate-100 hover:bg-slate-200 text-slate-700 px-5 py-3 rounded-xl font-semibold text-xs transition">
                Batal
            </a>
        </div>
    </form>
