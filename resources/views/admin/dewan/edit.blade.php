...
@section('title', 'Edit Data Tenaga Pendidik: ' . $dewan->name)
@section('header_title', 'Edit Data Dewan Guru & GTK: ' . $dewan->name)
...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <a href="{{ route('admin.dewan.index') }}" class="text-xs font-semibold text-slate-500 hover:text-slate-800 flex items-center space-x-2">
            ...
            <span>Kembali ke Daftar Dewan Guru</span>
        </a>
    </div>
        ...
        <div class="bg-white p-6 sm:p-8 rounded-3xl shadow-xs border border-slate-200/80 space-y-5">
                ...
                <div>
                    ...
                    @else
                        <select name="unit_pendidikan_id" id="unit_pendidikan_id" class="w-full bg-slate-50 text-xs text-slate-800 rounded-xl px-4 py-3 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#00913e]">
                            ...
                            @foreach($units as $u)
                                <option value="{{ $u->id }}" {{ old('unit_pendidikan_id', $dewan->unit_pendidikan_id) == $u->id ? 'selected' : '' }}>{{ $u->name }}</option>
                            @endforeach
                        </select>
                    @endif
                </div>
            ...
            <div>
                ...
                <div class="flex items-center space-x-4 mb-3">
                    <img src="{{ $dewan->photo }}" alt="{{ $dewan->name }}" class="w-16 h-16 rounded-xl object-cover border border-slate-200 shadow-xs" onerror="this.onerror=null;this.src='/uploads/logo-ppru-square.png'">
