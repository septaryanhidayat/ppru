...
<div class="space-y-6">
    <div class="bg-white p-6 sm:p-8 rounded-3xl shadow-xs border border-slate-200/80">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-100">
            ...
            <a href="{{ route('admin.dewan.create') }}" class="inline-flex items-center space-x-2 bg-[#0d6b38] hover:bg-[#094d28] text-white font-bold text-xs px-5 py-2.5 rounded-xl shadow-md transition self-start sm:self-auto">
                ...
                <div class="bg-slate-50 rounded-2xl p-5 border border-slate-200/80 flex flex-col justify-between space-y-4 hover:shadow-md transition">
                    <div class="text-center space-y-3">
                        <div class="w-24 h-24 mx-auto rounded-2xl bg-white p-1 shadow-xs border border-slate-200 overflow-hidden">
                            <img src="{{ $d->photo }}" alt="{{ $d->name }}" class="w-full h-full object-cover object-top rounded-xl" onerror="this.src='/uploads/logo-robbani-emblem.svg'">
                        </div>
                        <div>
                            <h3 class="font-extrabold text-sm text-slate-900">{{ $d->name }}</h3>
                            <span class="inline-block bg-emerald-100 text-[#0d6b38] text-[10px] font-bold px-2 py-0.5 rounded-full mt-1">
                                {{ $d->position }}
                            </span>
                            <p class="text-[11px] text-slate-500 mt-2 line-clamp-2">{{ $d->fraction ?? 'Dewan Guru & GTK Robbani' }}</p>
                        </div>
                    </div>
