            ...
            <div>
                <h2 class="text-lg font-black text-slate-800">
                    @if(auth()->user()?->isUnitAdmin())
                        Dewan Guru &amp; Tenaga Pendidik {{ auth()->user()->unit?->name }}
                    @else
                        Struktur Pengurus Yayasan &amp; Pendidik PPRU
                    @endif
                </h2>
                <p class="text-xs text-slate-500 mt-0.5">
                    @if(auth()->user()?->isUnitAdmin())
                        Kelola susunan dewan guru, foto resmi, jabatan, dan urutan tampil untuk unit {{ auth()->user()->unit?->short_name }}.
                    @else
                        Kelola susunan pimpinan yayasan (YAPIRUS), dewan guru, foto resmi, dan urutan tampil hirarki.
                    @endif
                </p>
            </div>
            
            <div class="flex flex-wrap items-center gap-3">
                @if(!auth()->user()?->isUnitAdmin() && $tree)
                ...
                <div class="bg-slate-50 rounded-2xl p-5 border border-slate-200/80 flex flex-col justify-between space-y-4 hover:shadow-md transition">
                    ...
                    <div class="pt-3 border-t border-slate-200 flex items-center justify-between text-xs">
                        ...
                        <div class="flex items-center space-x-2">
                            ...
                            @if($d->fraction === 'Yayasan')
                                <span class="p-2 text-amber-500/70" title="Pimpinan Yayasan (Dilindungi)">
