    ...
    <form action="{{ route('admin.unit-pendidikan.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="bg-white p-6 sm:p-8 rounded-3xl shadow-xs border border-slate-200/80 space-y-5">
            ...
            <div class="p-5 bg-emerald-50/40 rounded-2xl border border-emerald-100 space-y-3">
                ...
                <p class="text-[11px] text-slate-500">Unggah foto dokumentasi unit kegiatan atau gedung (JPG, PNG, WebP).</p>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1">
                    ...
                    <div>
                        <input type="text" name="thumbnail" id="thumbnail" value="{{ old('thumbnail') }}" placeholder="Atau URL / Path Foto (/uploads/...)" class="w-full bg-white text-xs text-slate-800 rounded-xl px-4 py-2.5 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#00843d] font-mono">
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="description" rows="5" placeholder="Tuliskan profil unit, visi pendidikan, kurikulum, fasilitas khas, dan prestasi..." class="w-full bg-slate-50 text-xs text-slate-800 rounded-xl p-4 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#00843d] leading-relaxed">{{ old('description') }}</textarea>
            </div>
