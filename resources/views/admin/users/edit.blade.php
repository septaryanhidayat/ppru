    ...
    <form action="{{ route('admin.users.update', $user) }}" method="POST" class="space-y-6">
        ...
        <div class="bg-white p-6 sm:p-8 rounded-3xl shadow-xs border border-slate-200/80 space-y-5">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" required value="{{ old('name', $user->name) }}" class="w-full bg-slate-50 text-xs text-slate-800 rounded-xl px-4 py-3 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#da251c] transition">
            ...
            <div>
                ...
                <input type="email" name="email" id="email" required value="{{ old('email', $user->email) }}" class="w-full bg-slate-50 text-xs text-slate-800 rounded-xl px-4 py-3 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#da251c] transition">
            ...
            <div>
                ...
                <input type="password" name="password" id="password" minlength="8" placeholder="••••••••" class="w-full bg-slate-50 text-xs text-slate-800 rounded-xl px-4 py-3 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#da251c] transition">
            ...
            <div>
                ...
                <select name="role" id="role" required onchange="toggleUnitSelect(this.value)" class="w-full bg-slate-50 text-xs text-slate-800 rounded-xl px-4 py-3 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#da251c] transition">
                    ...
                    <option value="admin_unit" {{ old('role', $user->role) === 'admin_unit' ? 'selected' : '' }}>Admin Unit Pendidikan - Akses terbatas terisolasi khusus 1 Unit Lembaga</option>
            ...
            <div id="unit-select-box" class="{{ old('role', $user->role) === 'admin_unit' ? '' : 'hidden' }}">
                ...
                <select name="unit_pendidikan_id" id="unit_pendidikan_id" class="w-full bg-emerald-50/50 text-xs text-slate-800 rounded-xl px-4 py-3 border border-emerald-300 focus:outline-none focus:ring-2 focus:ring-[#00843d] transition">
                    ...
                    @foreach($units as $u)
                        <option value="{{ $u->id }}" {{ (string) old('unit_pendidikan_id', $user->unit_pendidikan_id) === (string) $u->id ? 'selected' : '' }}>
                            {{ $u->name }} ({{ $u->short_name }})
                        </option>
                    @endforeach
                </select>
